...
function BindBranchDropdown() {
    $.ajax({
        type: "POST",
        url: 'wfHrEmpEsiStaturyReport.aspx/BranchMasterList',
        data: {},
...
function BindEmployeeFilter() {
    if ($('#ddl_Branch').val() != '') {
        $.ajax({
            type: "POST",
            url: 'wfHrEmpEsiStaturyReport.aspx/EmployeeMasterList',
            data: JSON.stringify({
...
function FetchEmployeeDetails(empid) {
    $.ajax({
        type: "POST",
        url: 'wfHrEmpEsiStaturyReport.aspx/FetchEmployeeDetails',
        data: JSON.stringify({
            "EmpId": empid
        }),
        contentType: "application/json; charset=utf-8",
...
function BindEmployeeSaleryApproveList(tbodyid, tbl_id, branchid, month, year, empid, SalaryType, SalaryApprove) {
    $.ajax({
        type: "POST",
        url: 'wfHrEmpEsiStaturyReport.aspx/FetchEmployeSalaryApproveList',
        data: JSON.stringify({
            "branchid": branchid,
            "month": month,
            "year": year,
        ...
        success: function (response) {
            ...
            var html = '';
            var esEmployerTotal = 0; // Sum of ESI Employer Value
            var esEmployeeTotal = 0; // Sum of ESI Employees Value
            for (var i = 0; i < data.length; i++) {
                html = html + '<tr>'
                    + '<td><input type="checkbox" class="editor-active" value="' + data[i].Id + '"></td>'
                    + '<td>' + (data[i].Year != undefined ? data[i].Year : '') + '</td>'
                    + '<td>' + (data[i].Month != undefined ? data[i].Month : '') + '</td>'
                    + '<td>' + data[i].EmpId + '</td>'
                    + '<td>' + data[i].EmpName + '</td>'                    
                    + '<td>' + (data[i].EsiNo != undefined ? data[i].EsiNo : '') + '</td>'
                    ...
                    + '</tr>';
                // Accumulate totals
                esEmployerTotal += parseFloat(data[i].ESI_EmployerValue) || 0;
                esEmployeeTotal += parseFloat(data[i].ESI_EmployeesValue) || 0;
            }
            ...
            if ($("#btnSave").is(":visible")) {
                    ...
                    buttons: [
                        {
                            ...
                            action: function (e, dt, button) {
                                ...
                                //var title = [["Employee Salary Details - " + d.toDateString()]];
                                ...
                                const totalsRow = [
                                    ...
                                    '',
                                    '',
                                    esEmployerTotal.toFixed(2), // ESI Employer Value total
                                    '',
                                    esEmployeeTotal.toFixed(2) // ESI Employees Value total
                                ];
                                ...
                                //worksheet['!merges'].push({ s: { r: 0, c: 0 }, e: { r: 0, c: headers.length - 1 } }); // Merge title

                                // Set title cell style
                                //const titleCell = worksheet['A1'];
                               // titleCell.s = { font: { bold: true, sz: 14 }, alignment: { horizontal: 'center', vertical: 'middle' } };
                                ...
                                XLSX.utils.book_append_sheet(workbook, worksheet, 'Emp Esi Report');

                                // Generate the Excel file and trigger download
                                XLSX.writeFile(workbook, "Employee_Esi_Details_" + d.toDateString() + ".xlsx");
                            }
                        }
                    ]
                });
                ...
                //            $.ajax({
                //                type: "POST",
                //                url: 'wfHrEmpEsiStaturyReport.aspx/SalaryExportToPDF',
                //                data: JSON.stringify({
                //                    "id": id
                //                }),
                //                contentType: "application/json; charset=utf-8",
...
function RejectSalaryApprove(id) {
    $.ajax({
        type: "POST",
        url: 'wfHrEmpEsiStaturyReport.aspx/RejectSalaryApprove',
        data: JSON.stringify({
            "id": id,
...
function UpdateSalaryApprove(id) {
    $.ajax({
        type: "POST",
        url: 'wfHrEmpEsiStaturyReport.aspx/UpdateSalaryApprove',
        data: JSON.stringify({
            "id": id,
            "loginuser": $('#ContentPlaceHolder1_loginuser').val()
        }),
