$(document).ready(function () {
    ...
    $.ajax({
        ...
        type: "POST",
        url: "wfMmMaterialPurchaseOrderDirectEntry_display.aspx/FetchPODirectDetails",
        data: JSON.stringify({
            "Id": Id
        }),
    ...
    function populateTable(data) {
        ...
        var tableHTML = '<table class="center" id="search_form" style="width:90%;border:solid 1px black;padding:5px;">'
            + '<tr><td colspan="2" style="text-align:center;"><h2>Direct Purchase Order</h2></td></tr>'
            ...
            + '</td><td><b>PurchaseOrderID :</b> ' + data.Table3[0].Id + '<br/><b>Order Entry Date :</b> ' + data.Table3[0].OrderDate + '</td></tr>'
            
            + '<tr><td colspan="2"><table style="width:100%;" id="search_form1">'
            + '<thead><tr><th style="display: none;">Id</th><th>Item</th><th>Qty</th><th>Unit Measure</th><th>Rate</th>'
            + '<th>Taxable Amt</th><th>CGST(%)</th><th>SGST(%)</th><th>IGST(%)</th><th>Freight Charge</th><th>Loading/Un Loading</th><th>Round Off</th><th>Gross Amt</th>'
            + '</tr></thead><tbody>';
                var total_gross = 0;
                var total_taxable = 0;
            for (var i = 0; i < data.Table4.length; i++) {
                ...
                tableHTML += '<tr><td style="display: none;">' + data.Table4[i].ItemId + '</td><td>'
                    + data.Table4[i].materialName + '</td><td>'
                    + data.Table4[i].Qty.toFixed(2) + '</td><td>'
                    + data.Table4[i].UnitMesure + '</td><td>'
                    + data.Table4[i].Rate.toFixed(2) + '</td><td>'
                    + data.Table4[i].TaxbleAmt.toFixed(2) + '</td><td>'
                    ...
                    + data.Table4[i].TotalAmt.toFixed(2) + '</td></tr>'
