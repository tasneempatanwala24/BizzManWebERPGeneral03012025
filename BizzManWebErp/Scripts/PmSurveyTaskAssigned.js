...
function BindCustomerList() {
    ...
    $.ajax({
        type: "POST",
        url: 'wfPmSurveyTaskAssigned.aspx/FetchTaskAssignedList',
        data: {},
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        beforeSend: function () {
        ...
        success: function (response) {
            ...
            var html = '';
            for (var i = 0; i < JSON.parse(response.d).length; i++) {
                //html = html + '<tr onclick="FetchActivityDetails(\'' + data[i].VisitId + '\')"><td>' + data[i].VisitId + '</td>'
                html = html 
                    + '<tr><td>' + data[i].VisitId + '</td>'
                    + '<td>' + data[i].DateOfVisit + '</td>'
                    + '<td>' + data[i].Time + '</td>'
                    + '<td>' + data[i].AssignedDate + '</td>'
                    + '<td>' + data[i].ClientId  + '</td></tr>'
                    //+ '<td>' + (data[i].AssignPerson != undefined ? data[i].AssignPerson : '') + '</td>'
...
function FetchActivityDetails(VisitId) {
