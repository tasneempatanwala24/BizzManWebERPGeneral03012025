...
function BindBranchDropdown() {
    $.ajax({
        type: "POST",
        url: 'wfHrEmpPfStaturyReport.aspx/BranchMasterList',
        data: {},
...
function BindEmployeeFilter() {
    if ($('#ddl_Branch').val() != '') {
        $.ajax({
            type: "POST",
            url: 'wfHrEmpPfStaturyReport.aspx/EmployeeMasterList',
            data: JSON.stringify({
                "branchid": $('#ddl_Branch').val()
            }),
            contentType: "application/json; charset=utf-8",
            dataType: "json",
            beforeSend: function () {
...
function FetchEmployeeDetails(empid) {
    $.ajax({
        type: "POST",
        url: 'wfHrEmpPfStaturyReport.aspx/FetchEmployeeDetails',
        data: JSON.stringify({
            "EmpId": empid
        }),
        contentType: "application/json; charset=utf-8",
...
//    if ($("#btnSave").is(":visible")) {
//        if ($('#ddlBranch').val() != '' || $('#ddl_Month').val() != '' || $('#txtYear').val() != '') {
//            if ($('#ddl_Month').val() != '') {
//                if ($('#txtYear').val() != '') {
//                    BindEmployeeSaleryApproveList('tbody_SalaryApproveListEmployeeWise', 'tblSalaryApproveEmployeeWise',
//                        $('#dd_Branch').val(), $('#ddl_Month').val(), $('#txtYear').val(), $('#ddl_Employee').val(),
//                        $('#ddlSalaryType').val(), 'N');
//                }
//                else {
//                    alertify.error('Please enter any year');
...
//        if ($('#ddl_Branch').val() != '' || $('#ddlMonth').val() != '' || $('#txtYear').val() != ''
//            || $('#ddl_Employee').val() != '') {
//            if (chkMonth == 1) {
//                if (chkYear == 1) {
//                    console.log($('#txtYear').val());
//                    BindEmployeeSaleryApproveList('tbody_Employee_SalaryApproveList', 'tblEmployeeSalaryApproveList',
//                        $('#ddl_Branch').val(), $('#ddlMonth').val(), $('#txtYear').val(), $('#ddl_Employee').val(),
//                        $('#ddl_SalaryType').val(), 'Y');
//                }
//                else {
//                    alertify.error('Please enter any year');
//                }
...
function BindEmployeeSaleryApproveList(tbodyid, tbl_id, branchid, month, year, empid, SalaryType, SalaryApprove) {
    ...
    console.log(SalaryApprove);
    $.ajax({
        type: "POST",
        url: 'wfHrEmpPfStaturyReport.aspx/FetchEmployeSalaryApproveList',
        data: JSON.stringify({
            "branchid": branchid,
            "month": month,
            "year": year,
        ...
        success: function (response) {
            ...
            var html = '';
            var pfEmployerTotal = 0;
            var pfEmployeeTotal = 0;
            var pfAdminTotal = 0;
            var pfEDLITotal = 0;
            for (var i = 0; i < data.length; i++) {
                html = html + '<tr><td><input type="checkbox" class="editor-active" value="' + data[i].Id + '"></td>'
                    ...
                    + '<td>' + (data[i].PfNo != undefined ? data[i].PfNo : '') + '</td>'
                    ...
                    + '<td>' + (data[i].EDLI_Percentage != undefined ? data[i].EDLI_Percentage : '') + '</td>'
                    + '<td>' + (data[i].EDLI_Amount != undefined ? data[i].EDLI_Amount : '') + '</td>'
                    ...
                    //+ '<td>' + (data[i].SalaryPayment != undefined ? data[i].SalaryPayment : '') + '</td>'
                    + '</tr>';
                // Accumulate totals
                pfEmployerTotal += parseFloat(data[i].PF_EmployerValue) || 0;
                pfEmployeeTotal += parseFloat(data[i].PF_EmployeesValue) || 0;
                pfAdminTotal += parseFloat(data[i].AdminChargesAmount) || 0;
                pfEDLITotal += parseFloat(data[i].EDLI_Amount) || 0;
            }
            $('#' + tbodyid).html(html);
            var d = new Date();
            if ($("#btnSave").is(":visible")) {
                    ...
                    buttons: [
                        {
                            ...
                            action: function (e, dt, button) {
                                ...
                                // Get the body data
                                var exportData = dt.buttons.exportData().body;

                                // Combine title, headers, and data into a single array
                                //XLSX.utils.sheet_add_aoa(worksheet, title, { origin: 'A1' }); // Add title
                                XLSX.utils.sheet_add_aoa(worksheet, [headers], { origin: 'A1' }); // Add headers

                                // Add body data
                                XLSX.utils.sheet_add_aoa(worksheet, exportData, { origin: 'A2' }); // Start adding data from row 3

                                // Create a totals row manually
                                const totalsRow = [
                                    '',
                                    '',
                                    '',
                                    ...
                                    '',
                                    '',
                                    pfEmployerTotal.toFixed(2),
                                    '',
                                    pfEmployeeTotal.toFixed(2),
                                    '',
                                    pfAdminTotal.toFixed(2),
                                    '',
                                    pfEDLITotal.toFixed(2)
                                ];

                                // Add the totals row at the end
                                XLSX.utils.sheet_add_aoa(worksheet, [totalsRow], { origin: -1 }); // Append at the end

                                // Apply header styles (bold) in row 2
                                for (let col = 0; col < headers.length; col++) {
                                    const cellRef = XLSX.utils.encode_cell({ c: col, r: 1 }); // Headers are in row 2
                                    const headerCell = worksheet[cellRef];
                                    headerCell.s = { font: { bold: true, sz: 12 } }; // Bold and size 12
                                }

                                // Append the worksheet to the workbook
                                XLSX.utils.book_append_sheet(workbook, worksheet, 'Emp PF Report');

                                // Generate the Excel file and trigger download
                                XLSX.writeFile(workbook, "Employee_PF_Details_" + d.toDateString() + ".xlsx");
                            }
                        }
                    ]
                });
                ...
                $('#btnExportPDF').click(function (event) {
                    ...
                    table.$('input[type="checkbox"]').each(function () {
                        ...
                        if (this.checked) {
                            ...
                            $.ajax({
                                type: "POST",
                                url: 'wfHrEmpPfStaturyReport.aspx/SalaryExportToPDF',
                                data: JSON.stringify({
                                    "id": id
                                }),
                                contentType: "application/json; charset=utf-8",
...
function RejectSalaryApprove(id) {
    $.ajax({
        type: "POST",
        url: 'wfHrEmpPfStaturyReport.aspx/RejectSalaryApprove',
        data: JSON.stringify({
            "id": id,
...
function UpdateSalaryApprove(id) {
    $.ajax({
        type: "POST",
        url: 'wfHrEmpPfStaturyReport.aspx/UpdateSalaryApprove',
        data: JSON.stringify({
            "id": id,
            "loginuser": $('#ContentPlaceHolder1_loginuser').val()
        }),
