<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="wfSdSalesOrderInvoice_display.aspx.cs" Inherits="BizzManWebErp.wfSdSalesOrderInvoice_display" %>
...
<head runat="server">
    ...
    <script>
        ...
        function GetPDFContent(id) {
            $.ajax({
                type: 'POST',
                url: 'wfSdSalesOrderInvoice_display.aspx/GetQuotationData',
                ...
                success: function (response) {
                    ...
                    content += '<h3 style="text-align: center; font-weight: bold; margin-bottom: 20px;">Sales Order Invoice</h3>';
                    ...
                    if (clientDetails && clientDetails.length > 0) {
                        ...
                        if (quotationDetails && quotationDetails.length > 0) {
                            content += '<div style="text-align: right;">';
                            content += '<p style="margin: 5px 0;">Sales Invoice ID: ' + currentQuotationId + '</p>';
                            content += '<p style="margin: 5px 0;">Sales Invoice Date: ' + quotationDetails[0].SalesInvoiceDate + '</p>';
                            content += '<p style="margin: 5px 0;">Sales Order ID: ' + quotationDetails[0].SalesOrderID + '</p>';
                            content += '</div>';
                        }
                        content += '</div>';
                    }
                    ...
                    for (var i = 0; i < salesQuotationDetail.length; i++) {
                        ...
                        content += '<td style="border: 1px solid #000; padding: 8px;">' + parseFloat(salesQuotationDetail[i].GST).toFixed(2) + '</td>';
                        ...
                        debugger;
                        centralTaxValue += isNaN(salesQuotationDetail[i].CentralTaxValue) ? 0 : salesQuotationDetail[i].CentralTaxValue;
                        stateTaxValue += isNaN(salesQuotationDetail[i].StateTaxValue) ? 0 : salesQuotationDetail[i].StateTaxValue;
                        cessTaxValue += isNaN(salesQuotationDetail[i].CessValue) ? 0 : salesQuotationDetail[i].CessValue;
                    }
                    ...
                    if (quotationDetails && quotationDetails.length > 0) {
                        ...
                        content += '<td  style="text-align:left; border: 1px solid #000; font-style: oblique;font-size:large; padding:8px;">' + Math.round(parseFloat(quotationDetails[0].NetTotal - amountDetails[0]?.TotalPaid ).toFixed(2)) + '</td>';
                        content += '</tr>';
                    }
                    content += '</tfoot>';
                    content += '</table>';
...
<body>
    ...
    <form id="form1" runat="server">
        <table class="center" style="width: 60%; float:left!important" border="0">
            <tr>
